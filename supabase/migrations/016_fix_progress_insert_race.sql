-- ==============================================================================
-- 16. Fix Progress Insert Race Condition
-- ==============================================================================

-- Redefine the function with User-level locking to serialize concurrent requests
...
DECLARE
  ...
  v_delta INTEGER := 0;
BEGIN
  -- 0. Serialize execution for this user to prevent Insert Race Conditions
  -- This prevents two concurrent requests from both seeing "No Row" and double-counting stats.
  -- Acquiring a row lock on the user ensures that only one progress update runs at a time for this user.
  PERFORM 1 FROM users WHERE id = p_user_id FOR UPDATE;

  -- 1. Lock & Get Previous State
  -- FOR UPDATE ensures that concurrent requests for the same reading wait (if row exists)
